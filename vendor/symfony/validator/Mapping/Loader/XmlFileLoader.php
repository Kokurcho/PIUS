...
use Symfony\Component\Config\Util\Xmltools;
...
class XmlFileLoader extends FileLoader
{
    ...
    protected function parseConstraints(\SimpleXMLElement $nodes): array
    {
        ...
        foreach ($nodes as $node) {
            if (\count($node) > 0) {
                ...
                $options = Xmltools::phpize(trim($node));
            } else {
                $options = null;
            }
    ...
    {
        try {
            $dom = Xmltools::loadFile($path, __DIR__.'/schema/dic/constraint-mapping/constraint-mapping-1.0.xsd');
        } catch (\Exception $e) {
